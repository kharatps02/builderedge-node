...
export class SyncController {
      ...
      if (accessToken && vanityKey && appUrl) {
        ...
        this.orgMasterModel.getOrgConfigByVanityId(vanityKey, (error, config) => {
          if (!error && config) {
            ...
            let sfResponsePerRequest: ISFResponse<IProjectDetails> = new SFResponse<IProjectDetails>();

            const url = sfResponsePerRequest.nextRecordsUrl || config.api_base_url + Constants.SYNC_QUERIES.ALL;
            const func = (sfUrl: string, callbackFunc: (formattedStuff: any) => void) => {
                  ...
                  if (sfResponse.statusCode === 401) {
                    ...
                    if (sfResponse.body && sfResponse.body.length > 0) {
                      if (typeof sfResponse.body === 'string') {
                        sfResponsePerRequest = JSON.parse(sfResponse.body) as ISFResponse<IProjectDetails>;
                      } else {
                        sfResponsePerRequest = JSON.parse(sfResponse.body);
                      }

                      const formatedProjects = formatProjectAndTaskDetails(sfResponsePerRequest.records);
              ...
              this.syncDataModel.syncSalesforceUserDetails(
                { vanity_id: vanityKey, session_id: accessToken }, formatedProjects, (done) => {
                  if (done && sfResponsePerRequest.done) {
                    ...
                    func(sfResponsePerRequest.nextRecordsUrl!, (formattedStuff: any) => {
                      return;
                    });
                    // res.render('data-sync', { appUrl });
                    //     res.render('data-sync', { appUrl }, (err, html) => {
   ...
   * syncFirst
   */
  public async syncFirst(req: express.Request, res: express.Response, next: express.NextFunction) {
    const accessToken = req.body.accessToken;
    const vanityKey = req.params.vanityKey;
    const appUrl = req.body.appUrl;
    try {
      const config = await this.orgMasterModel.getOrgConfigByVanityId(vanityKey);
      let sfResponsePerRequest: ISFResponse<IProjectDetails> = new SFResponse<IProjectDetails>();
      const url = sfResponsePerRequest.nextRecordsUrl || config.api_base_url + Constants.SYNC_QUERIES.ALL;
      sfResponsePerRequest = await this.getData<IProjectDetails>(url, accessToken);
    } catch (error) {

    }

  }
  private async getData<T>(url: string, accessToken: string, records?: T[]): Promise<ISFResponse<T>> {
    const requestHeader = {
      headers: {
        'Authorization': 'Bearer ' + accessToken,
        'Content-Type': 'application/json',
