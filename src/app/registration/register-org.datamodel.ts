...
export class RegisterOrgDataModel {
     ...
     * oAuthCallback
     */
    public async oAuthCallback(encryptedORGID, encryptedRefreshToken, instanceUrl, grantedUserId) {
        ...
        try {
            ...
            if (result.rows.length === 0) {
                ...
                const queryStr = "INSERT INTO org_master(org_id, refresh_token, api_base_url, user_id) values($1, $2, $3, $4, $5, $6, $7) RETURNING vanity_id";
                const queryRes = await pgClient.query(queryStr, [encryptedORGID, encryptedRefreshToken, instanceUrl, grantedUserId]);
     ...
     */
    public async registeredSuccessfully(vanityurltext): Promise<any> {
        const pgClient = new Client(Constants.POSTGRES_DB_CONFIG);
        try {
            await utils.connectClient(pgClient);
            const queryStr = "SELECT * FROM org_master WHERE vanity_id = $1";
            const result = await pgClient.query(queryStr, [vanityurltext]);
            // if user is valid
            if (result.rows.length > 0) {
                ...
                // put some timeout/wait until the data is synched and then redirect.
